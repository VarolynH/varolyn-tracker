/* ═══════════════════════════════════════════════════════
 *  Varolyn Healthcare — Service Worker v5
 *  Unstoppable background tracking, offline buffer, push recovery,
 *  auto-resume, persistent notification, self-healing
 * ═══════════════════════════════════════════════════════ */

const CACHE = 'varolyn-v5';
...
self.addEventListener('message', async (e) => {
  ...
  if (type === 'SHOW_PERSISTENT_NOTIFICATION') {
    self.registration.showNotification('Varolyn Healthcare', {
      body: 'Location tracking is active. Do not dismiss.',
      icon: '/favicon.ico',
      badge: '/favicon.ico',
...
self.addEventListener('push', (e) => {
  ...
  const options = {
    ...
    data: { url: '/', token: payload.token || null, action: payload.action || 'resume' },
  ...
  e.waitUntil(
    self.registration.showNotification(payload.title || 'Varolyn Healthcare', options)
      .then(async () => {
        // Also try to wake up any existing client
...
// ── Notification click — reopen or focus app ──
self.addEventListener('notificationclick', (e) => {
  e.notification.close();

  // Don't close persistent notification when clicked
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(async (clientList) => {
      ...
      // No tab open — open new one (auto-resume happens via localStorage)
      if (clients.openWindow) return clients.openWindow('/');
    })
  );
});
...
self.addEventListener('periodicsync', (e) => {
  if (e.tag === 'varolyn-location-sync') e.waitUntil(syncBufferedLocations());
});

// ── Background Sync (when back online) ──
self.addEventListener('sync', (e) => {
  if (e.tag === 'varolyn-sync') e.waitUntil(syncBufferedLocations());
});
